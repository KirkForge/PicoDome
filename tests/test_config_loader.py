"""Tests for irondome.config — YAML loader, env overrides, CLI merge."""
...
class TestFindConfig:
    def test_finds_yml(self, tmp_path):
        (tmp_path / ".irondome.yml").write_text("format: json\n")
        result = _find_config(tmp_path)
        assert result is not None
        assert result.name == ".irondome.yml"

    def test_finds_yaml(self, tmp_path):
        (tmp_path / ".irondome.yaml").write_text("format: json\n")
        result = _find_config(tmp_path)
        assert result is not None
        assert result.name == ".irondome.yaml"

    def test_yml_precedence_over_yaml(self, tmp_path):
        (tmp_path / ".irondome.yml").write_text("format: json\n")
        (tmp_path / ".irondome.yaml").write_text("format: sarif\n")
        result = _find_config(tmp_path)
        assert result.name == ".irondome.yml"
...
class TestLoadConfigYAML:
    ...
    def test_invalid_log_format_ignored(self, tmp_path):
        (tmp_path / ".irondome.yml").write_text("log_format: xml\n")
        cfg = load_config(tmp_path)
        assert cfg.log_format == "text"
