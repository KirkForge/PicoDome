...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="irondome",
        description="Iron Dome — deterministic runtime sandbox and behavioral \
            analysis",
    )
    parser.add_argument(
        "--version"
        action="version"
        version=f"irondome {__version__}"
    )
    ...
    sandbox_parser = sub.add_parser(
        "sandbox"
        help="Run a command under L3 sandbox policy"
    )
    ...
    sandbox_parser.add_argument(
        ...
        "-p"
        type=Path
        help="Policy file (default: built-in)"
    )
    sandbox_parser.add_argument(
        "--timeout"
        "-t"
    ...
    analyze_parser = sub.add_parser(
        "analyze"
        help="Run L4 behavioral analysis on L3 output"
    )
    analyze_parser.add_argument(
        "--input"
        "-i"
        type=Path
        help="JSON file from 'irondome sandbox --format json'"
    )
    analyze_parser.add_argument(
        "--format", "-f",
        choices=["json", "sari", "table", "ml-context", "github", "cyclonedx"],
    ...
    pipeline_parser = sub.add_parser(
        "pipeline"
        help="Run full L3+L4 pipeline on a command"
    )
    ...
    pipeline_parser.add_argument(
        ...
        "-r"
        nargs="*"
        help="Specific L4 rule IDs to run"
    )
    ...
    rules_parser = sub.add_parser(
        "rules"
        help="List available L4 detector rules"
    )
    ...
    daemon_parser = sub.add_parser(
        "daemon"
        help="Start Iron Dome daemon (HTTP API server)"
    )
    ...
    scan_grpc_parser.add_argument(
        "--tls-cert"
        help="Client TLS certificate path"
    )
    scan_grpc_parser.add_argument("--tls-key", help="Client TLS key path")
    scan_grpc_parser.add_argument(
        "--tls-ca"
        help="CA certificate path for mTLS"
    )
    scan_grpc_parser.add_argument(
        "--retries"
        type=int
    ...
    audit_parser.add_argument(
        "--since"
        help="Events after this ISO timestamp"
    )
    audit_parser.add_argument(
        "--until"
        help="Events before this ISO timestamp"
    )
    audit_parser.add_argument(
        "--limit"
        type=int
    ...
    retention_parser = sub.add_parser(
        "retention"
        help="Manage data retention"
    )
    retention_parser.add_argument(
        "action"
        choices=["cleanup", "stats", "export"]
    ...
    # ── policy-versioned ──────────────────────────────────────────────
    policy_v_parser = sub.add_parser(
        "policy-versions"
        help="Manage versioned policies"
    )
    policy_v_parser.add_argument(
        "action"
        choices=["list", "show", "dif", "rollback", "verify"]
        help="Policy action"
    )
    ...
    policy_v_parser.add_argument(
        "--version-a"
        type=int
        help="First version for diff"
    )
    policy_v_parser.add_argument(
        "--version-b"
        type=int
        help="Second version for diff"
    )
    policy_v_parser.add_argument(
        "--author"
        default="cli-user"
        help="Author for rollback"
    )
    ...
    # ── notary ────────────────────────────────────────────────────────
    notary_parser = sub.add_parser(
        "notary"
        help="Audit transparency notary (Rekor/Sigstore)"
    )
    notary_sub = notary_parser.add_subparsers(
        dest="notary_command"
        help="notary sub-commands"
    )

    notary_submit = notary_sub.add_parser(
        "submit"
        help="Submit an audit entry to the notary"
    )
    notary_submit.add_argument(
        "--entry"
        type=Path
        required=True
        help="JSON file with the entry to notarize"
    )
    notary_submit.add_argument(
        "--notary",
        choices=["null", "rekor"],
    ...
    notary_submit.add_argument(
        "--hmac-key"
        default=None
        help="HMAC key (default: built-in)"
    )

    notary_verify = notary_sub.add_parser(
        "verify"
        help="Verify an audit entry against the notary"
    )
    notary_verify.add_argument(
        "--uuid"
        required=True
        help="UUID of the entry to verify"
    )
    notary_verify.add_argument(
        "--entry"
        type=Path
        required=True
        help="JSON file with the entry to verify"
    )
    notary_verify.add_argument(
        "--notary",
        choices=["null", "rekor"],
    ...
    cluster_parser = sub.add_parser(
        "cluster"
        help="Manage daemon cluster mode"
    )
    cluster_sub = cluster_parser.add_subparsers(
        dest="cluster_action"
        help="cluster sub-commands"
    )

    # cluster join
    cluster_join = cluster_sub.add_parser(
        "join"
        help="Join a cluster via peer address"
    )
    ...
    cluster_join.add_argument(
        ...
        type=int
        default=8444
        help="Local cluster port (default: 8444)"
    )
    cluster_join.add_argument(
        "--node-id"
    ...
    cluster_join.add_argument(
        "--heartbeat-interval", type=int, default=10,
        help="Heartbeat interval in seconds (default: 10)",
    )
    cluster_join.add_argument(
        "--heartbeat-timeout", type=int, default=30,
    ...
    cluster_status = cluster_sub.add_parser(
        "status"
        help="Show cluster node status"
    )
    cluster_status.add_argument("--format", "-", choices=["json", "table"], \
        default="table",
                                help="Output format (default: table)")

    # cluster leave
    _cluster_leave = cluster_sub.add_parser(
        "leave"
    ...
    init_parser = sub.add_parser(
        "init"
        help="Initialize Iron Dome configuration"
    )
    ...
    if hasattr(args, 'verify_determinism') and args.verify_determinism:
        ...
        if not args.quiet:
            if is_match:
                ...
                print(
                    f"✗ Determinism FAILED: {hash_a} != {hash_b}"
                    file=sys.stderr
                )
        if not is_match:
            return 4
...
def _cmd_pipeline(args) -> int:
    ...
    if not args.quiet:
        ...
        if args.summary:
            ...
            print(
                format_pipeline_json(sandbox, analysis, \
                    deterministic=deterministic)
                )
        elif fmt == "sarif":
...
def _cmd_diff(args) -> int:
    ...
    exit_code, message = diff_results(
        args.file_a
        args.file_b
...
def _cmd_daemon(args) -> int:
    ...
    if transport == "grpc":
        from irondome.grpc_transport import (
            IronDomeGRPCServer,
            is_grpc_available,
        )
        ...
        if not is_grpc_available():
            print(
                "Error: grpcio is not installed. Install with: pip install \
                    grpcio"
                file=sys.stderr
            )
            return 1
...
def _cmd_scan_grpc(args) -> int:
    ...
    if not is_grpc_available():
        print(
            "Error: grpcio is not installed. Install with: pip install grpcio"
            file=sys.stderr
        )
        return 1
...
def _cmd_health(args) -> int:
    ...
    if args.format == "json":
        ...
        print(
            f"\n{icon} Iron Dome Health: {'HEALTHY' if all_healthy else \
                'UNHEALTHY'}\n"
            )
        for c in checks:
...
def _cmd_audit(args) -> int:
    ...
    for evt in events:
        print(
            f"[{evt.timestamp}] {evt.event_type.value} actor={evt.actor} \
                target={evt.target}"
            )
        if evt.detail:
            print(f"  {evt.detail}")
...
def _cmd_policy_versions(args) -> int:
    ...
    if args.action == "list":
        ...
        if not args.name or args.version_a is None or args.version_b is None:
            print(
                "--name, --version-a, and --version-b are required for 'diff'"
                file=sys.stderr
            )
            return 1
        ...
        if not args.name or args.version is None:
            print(
                "--name and --version are required for 'rollback'"
                file=sys.stderr
            )
            return 1
        ...
        if pv is None:
            ...
            return 1
        print(
            f"Rolled back '{args.name}' to v{args.version} → new v{pv.version}"
            )
        return 0
...
def _cmd_notary(args) -> int:
    ...
    if args.notary_command == "submit":
        if not args.entry or not args.entry.exists():
            print(
                f"Error: entry file not found: {args.entry}"
                file=sys.stderr
            )
            return 1
        ...
        try:
            ...
            if verified:
                ...
                print(
                    f"✗ Entry {args.uuid} verification FAILED"
                    file=sys.stderr
                )
                return 1
        except Exception as exc:
...
def _cmd_cluster(args) -> int:
    ...
    if action == "join":
        ...
        # Select backend
        backend = MemoryStateBackend() if args.backend == "memory" else \
            SQLiteStateBackend()
        ...
        print(
            f"  Nodes: {status['nodes_online']} online, \
        ...
        if args.format == "json":
            ...
            if status["nodes"]:
                print(
                    f"  {'Node ID':<30} {'Address':<20} {'Port':<6} \
                        {'Status':<10} {'Load':<5} {'Last HB'}"
                    )
                print(
                    f"  {'─' * 30} {'─' * 20} {'─' * 6} {'─' * 10} {'─' * 5} \
                        {'─' * 20}"
...
def _output_summary_pipeline(
    ...
    print(
        f"L3: {l3_verdict} ({events} events) → L4: {l4_verdict} ({findings} \
            findings) | {cmd}"
        )
