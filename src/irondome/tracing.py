...
Usage in daemon mode::
    ...
    from irondome.tracing import get_tracer, trace_scan
...
Configuration:
    IRONDOME_TRACING_ENABLED — set to "1" to enable tracing (default: off)
    IRONDOME_TRACING_EXPORTER — "otlp" or "none" (default: "none")
...
logger = logging.getLogger("irondome.tracing")
...
_tracing_enabled = os.environ.get("IRONDOME_TRACING_ENABLED", "").lower() in ("1", "true", "yes")
...
def trace_scan(command: list[str], backend: str = "", **attrs: Any):
    ...
    try:
        if not isinstance(span, _NoopSpan):
            span.set_attribute("irondome.command", " ".join(command))
            if backend:
                span.set_attribute("irondome.backend", backend)
            for key, value in attrs.items():
                span.set_attribute(f"irondome.{key}", str(value))
        yield span
    except Exception:
        if not isinstance(span, _NoopSpan):
...
def trace_daemon_request(method: str, path: str, request_id: str = "", **attrs: Any):
    ...
    try:
        if not isinstance(span, _NoopSpan):
            ...
            if request_id:
                span.set_attribute("irondome.request_id", request_id)
            for key, value in attrs.items():
                span.set_attribute(f"irondome.{key}", str(value))
        yield span
    finally:
